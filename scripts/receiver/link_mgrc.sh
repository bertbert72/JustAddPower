...
to_s_pre_idle()
{
	...
	if [ "$ACCESS_ON" = 'y' ]; then
		# Bruce171225. For the case that "eth off/on in a short time", lp service
		# is may not stopped yet. and STATE will stay in 's_pre_idle' forever.
		# Add a check and do_stop_srv_lp() here to resolve this issue.
		if ! is_all_stopped_lp ; then
			do_stop_srv_lp
		fi
		# Bruce171225. For the case that "eth off/on in a short time", we don't
		# want to switch back to GUI screen. So, start_link_off_timer() called
		# with $LM_LINK_OFF_TIMEOUT is still better then fixed 5 seconds timeout.
		##########
