...
#	watchdog.sh &
	...
	# stop heartbeat
	pkill -9 ast_heartbeat 2>/dev/null
	# Bruce160304. If we 'e_reconnect' very fast, there may has an 'heartbeat_ini_ok' event queued in this state.
	# We can't directly set HEARTBEAT_RDY='n' here otherwise HEARTBEAT_RDY will be set to 'y' again in the queued
	# heartbeat_ini_ok event handler. We send e_heartbeat_stopped event to resolve this issue.
...
handle_e_attaching_()
{
	...
	if [ "$STATE" = 's_srv_on' ] || [ "$STATE" = 's_attaching' ] ; then
		...
		if [ "$MULTICAST_ON" = 'y' ]; then
			...
			if [ "$SHARE_USB_ON_FIRST_PEER" = 'y' ] && [ "$CLIENT_NUM" -eq '1' ]; then
...
state_machine()
{
	...
	while event="$(< $PIPE)"; do
		...
		#for event in "$events"; do
			...
			# Bruce160406. Looks like 'printf' take much more CPU time than 'echo'. Rollback to 'echo' method.
