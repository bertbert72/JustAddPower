...
#	ANALOG_ONLY='n'
  ...
  # for web page display
	EDID_MODE_SELECT=`astparam g edid_rom_mode`
	if echo "$EDID_MODE_SELECT" | grep -q "not defined" ; then
		EDID_MODE_SELECT=`astparam r edid_rom_mode`
		if echo "$EDID_MODE_SELECT" | grep -q "not defined" ; then
			EDID_MODE_SELECT='HDMI'
		fi
	fi	          
...
echo_parameters_json()
{
	...
	# EDID Selection
	echo "\"EDID_MODE_SELECT\":\"$EDID_MODE_SELECT\"," 
	# End
	echo "\"_result\":\"pass\""
	echo "}"
	sleep 0.01
